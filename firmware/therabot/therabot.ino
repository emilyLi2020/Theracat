...
enum MotorMode { MODE_IDLE, MODE_CALM, MODE_ATTENTION, MODE_OFF };
...
class CmdCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic* c) {
    ...
    char cmd = v[0];
    switch (cmd) {
      case 'C': motorMode = MODE_CALM;      modeUntil = millis() + 20000; Serial.println("[motor] CALM");      break;
      case 'A': motorMode = MODE_ATTENTION; modeUntil = millis() + 3000;  Serial.println("[motor] ATTENTION"); break;
      case 'O': motorMode = MODE_OFF;       modeUntil = 0;                Serial.println("[motor] OFF");       break;
      case 'I':
      default:  motorMode = MODE_IDLE;      modeUntil = 0;                Serial.println("[motor] IDLE");      break;
    }
    motorPhase = 0;
...
void motorStop()     { digitalWrite(MOTOR_ENA, LOW);  digitalWrite(MOTOR_IN1, LOW);  digitalWrite(MOTOR_IN2, LOW);  }
...
void updateMotor() {
  ...
  if (motorMode == MODE_OFF) { motorStop(); return; }

  unsigned long now = millis();

  if (motorMode == MODE_IDLE) {
    const unsigned long PH = 5000;
    if (now - motorPhaseStart >= PH) { motorPhase = (motorPhase + 1) % 4; motorPhaseStart = now; }
    switch (motorPhase) {
      case 0: motorForward(); break;
      case 1: motorStop();    break;
      case 2: motorReverse(); break;
      case 3: motorStop();    break;
    }
  } else if (motorMode == MODE_CALM) {
    const unsigned long ON = 2000, OFF = 3000;
    if (motorPhase == 0 && now - motorPhaseStart >= ON)  { motorPhase = 1; motorPhaseStart = now; }
    if (motorPhase == 1 && now - motorPhaseStart >= OFF) { motorPhase = 0; motorPhaseStart = now; }
    if (motorPhase == 0) motorForward(); else motorStop();
...
void loop() {
  ...
  if (now - lastSampleMs >= SAMPLE_PERIOD_MS) {
    ...
    batchBuf[batchIdx][0] = fx;
    batchBuf[batchIdx][1] = fy;
    batchBuf[batchIdx][2] = fz;
    batchIdx++;
